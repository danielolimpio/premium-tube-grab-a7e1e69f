...
import { Download, Clipboard, Music, Smartphone, FolderOpen, Play } from "lucide-react";
      ...
      <div className="relative max-w-4xl mx-auto text-center">
        ...
        >
          ...
          >
            ...
            <button
              onClick={handlePaste}
              className="flex items-center gap-1.5 mr-3 px-4 py-2 rounded-lg bg-primary/10 text-primary text-sm font-semibold hover:bg-primary/20 hover:scale-105 transition-premium flex-shrink-0"
            >
        ...
        <motion.button
          ...
          whileTap={{ y: -1 }}
          className="gradient-red text-primary-foreground px-10 py-4 rounded-2xl text-base font-bold tracking-wide uppercase shadow-button hover:shadow-button-hover transition-premium inline-flex items-center gap-3 mb-8"
        >
        ...
        >
          {[
            { icon: Smartphone, label: "Shorts" },
            { icon: Music, label: "Extrair MP3" },
            { icon: FolderOpen, label: "Playlist" },
          ].map((item) => (
            <button
              key={item.label}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-border/50 text-muted-foreground text-sm font-medium hover:border-primary hover:text-foreground hover:bg-primary/5 transition-premium"
            >
